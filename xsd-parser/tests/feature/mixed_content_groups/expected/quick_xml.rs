...
pub mod quick_xml_deserialize {
    ...
    impl<'de> Deserializer<'de, super::NormalType> for NormalTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                ...
                S::Group(None) => Some(S::Group(None)),
                S::Baz(None) => Some(S::Baz(None)),
                _ => None,
            };
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::NormalGroupType as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::MixedType> for MixedTypeDeserializer {
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::MixedGroupType as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }

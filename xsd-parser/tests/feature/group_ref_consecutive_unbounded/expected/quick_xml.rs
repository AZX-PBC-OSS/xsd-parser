...
pub mod quick_xml_deserialize {
    ...
    impl<'de> Deserializer<'de, super::RecordType> for RecordTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                ...
                S::Contact(None) => Some(S::Contact(None)),
                S::Address(None) => Some(S::Address(None)),
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::RecordContactType as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
            if <super::RecordAddressType as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::RecordContactType> for RecordContactTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                S::Init__ => Some(S::Init__),
                S::Email(None) => Some(S::Email(None)),
                _ => None,
            };
            let (event, allow_any) = loop {
                let state = replace(&mut *self.state__, S::Unknown__);
    ...
    impl<'de> Deserializer<'de, super::RecordAddressType> for RecordAddressTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                S::Init__ => Some(S::Init__),
                S::Street(None) => Some(S::Street(None)),
                S::City(None) => Some(S::City(None)),
                _ => None,
            };
            let (event, allow_any) = loop {
                let state = replace(&mut *self.state__, S::Unknown__);

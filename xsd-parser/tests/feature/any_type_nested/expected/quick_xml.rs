...
pub mod quick_xml_deserialize {
    ...
    impl<'de> Deserializer<'de, super::RootType> for RootTypeDeserializer {
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if x.name().local_name().as_ref() == b"Container" {
                return true;
            }
            false
        }
    ...
    impl<'de> Deserializer<'de, super::ContainerType> for ContainerTypeDeserializer {
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::ContainerTypeContent as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::ContainerTypeContent> for ContainerTypeContentDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                S::Init__ => Some(S::Init__),
                S::Known(None) => Some(S::Known(None)),
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if x.name().local_name().as_ref() == b"Known" {
                return true;
            }
            false
        }

...
pub mod quick_xml_deserialize {
    ...
    impl<'de> Deserializer<'de, super::ContainerType> for ContainerTypeDeserializer {
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::ContainerContent2Type as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::ContainerContent2Type> for ContainerContent2TypeDeserializer {
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            ...
            if <super::ContainerContent3Type as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::ContainerContent3Type> for ContainerContent3TypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                S::Init__ => Some(S::Init__),
                S::Content4(None) => Some(S::Content4(None)),
                _ => None,
            };
            let (event, allow_any) = loop {
                let state = replace(&mut *self.state__, S::Unknown__);
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if <super::ContainerContent4Type as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }

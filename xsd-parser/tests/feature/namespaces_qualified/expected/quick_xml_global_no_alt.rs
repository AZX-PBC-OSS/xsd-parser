    ...
    impl<'de> Deserializer<'de, super::FooType> for FooTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                ...
                S::Inner1(None) => Some(S::Inner1(None)),
                S::Inner2(None) => Some(S::Inner2(None)),
                _ => None,
            };
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            let _ = helper;
            if matches!(
                helper.resolve_local_name(x.name(), &super::NS_UNNAMED_6),
                Some(b"Inner1")
            ) {
                return true;
            }
            if matches!(
                helper.resolve_local_name(x.name(), &super::NS_UNNAMED_7),
                Some(b"Inner2")
            ) {
                return true;
            }

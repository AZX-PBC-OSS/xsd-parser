...
pub mod quick_xml_deserialize {
    ...
    impl<'de> Deserializer<'de, super::CatalogType> for CatalogTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                ...
                S::Entry(None) => Some(S::Entry(None)),
                _ => None,
            };
        ...
        fn is_known_start_tag(helper: &DeserializeHelper, x: &BytesStart<'_>) -> bool {
            ...
            if <super::CatalogEntryType as WithDeserializer>::Deserializer::is_known_start_tag(
                helper, x,
            ) {
                return true;
            }
    ...
    impl<'de> Deserializer<'de, super::CatalogEntryType> for CatalogEntryTypeDeserializer {
        ...
        fn next(
            ...
            let entry_state__ = match &*self.state__ {
                S::Init__ => Some(S::Init__),
                S::Key(None) => Some(S::Key(None)),
                S::Value(None) => Some(S::Value(None)),
                _ => None,
            };
            let (event, allow_any) = loop {
                let state = replace(&mut *self.state__, S::Unknown__);
